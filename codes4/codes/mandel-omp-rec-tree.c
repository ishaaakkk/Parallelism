...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      y = start_fil;
      x = start_col;
      int ref = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI,
                        x, y, scale_real, scale_imag, maxiter);

      M[y][x] = ref;

      int equal_vertical = non_first_call;
      int equal_horizontal = non_first_call;

      #pragma omp task firstprivate(M, NRows, NCols, x, y, ref, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter) shared(equal_vertical)
      {
      for (int py = y + 1; py < y + NRows - 1; py++) {
            M[py][x] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI,
                                    x, py, scale_real, scale_imag, maxiter);

            M[py][x + NCols - 1] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI,
                                                x + NCols - 1, py, scale_real, scale_imag, maxiter);

            equal_vertical = equal_vertical & (ref == M[py][x]);
            equal_vertical = equal_vertical & (ref == M[py][x + NCols - 1]);
      }
      }

      #pragma omp task firstprivate(M, NRows, NCols, x, y, ref, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter) shared(equal_horizontal)
      {
      for (int px = x; px < x + NCols; px++) {
            M[y][px] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI,
                                    px, y, scale_real, scale_imag, maxiter);

            M[y + NRows - 1][px] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI,
                                                px, y + NRows - 1, scale_real, scale_imag, maxiter);

            equal_horizontal = equal_horizontal & (ref == M[y][px]);
            equal_horizontal = equal_horizontal & (ref == M[y + NRows - 1][px]);
      }
      }

      #pragma omp taskwait

      equal = equal_vertical & equal_horizontal;
...
#pragma omp atomic
		  ...
		  for (int py = y; py < y + NRows; py++)
			for (int px = x; px < x + NCols; px++) {
			      M[py][px] = M[y][x];
			      if (output2display)
#pragma omp critical(display_output)
                        {
...
#pragma omp critical(display_output)
            ...
            int y0 = start_fil;
            int y1 = start_fil + hRows;
            int x0 = start_col;
            int x1 = start_col + hCols;

#pragma omp task firstprivate(M, hRows, hCols, y0, x0, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
            mandel_tiled_rec(M, hRows, hCols, y0, x0,
                              CminR, CminI, CmaxR, CmaxI,
                              scale_real, scale_imag, maxiter);

#pragma omp task firstprivate(M, hRows, hCols, y1, x0, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
            mandel_tiled_rec(M, hRows, hCols, y1, x0,
                              CminR, CminI, CmaxR, CmaxI,
                              scale_real, scale_imag, maxiter);

#pragma omp task firstprivate(M, hRows, hCols, y0, x1, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
            mandel_tiled_rec(M, hRows, hCols, y0, x1,
                              CminR, CminI, CmaxR, CmaxI,
                              scale_real, scale_imag, maxiter);

#pragma omp task firstprivate(M, hRows, hCols, y1, x1, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
            mandel_tiled_rec(M, hRows, hCols, y1, x1,
                              CminR, CminI, CmaxR, CmaxI,
                              scale_real, scale_imag, maxiter);
            } else {
            int hCols = NCols / 2;
...
#pragma omp task firstprivate(M, NRows, hCols, y0, x0, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
            mandel_tiled_rec(M, NRows, hCols, y0, x0,
                              CminR, CminI, CmaxR, CmaxI,
                              scale_real, scale_imag, maxiter);

#pragma omp task firstprivate(M, NRows, hCols, y0, x1, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
            mandel_tiled_rec(M, NRows, hCols, y0, x1,
                              CminR, CminI, CmaxR, CmaxI,
                              scale_real, scale_imag, maxiter);
            }
	    }
