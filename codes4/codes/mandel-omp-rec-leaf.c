...
#pragma omp task firstprivate(M, NRows, NCols, y, x, value)
            {
...
#pragma omp critical(display_output)
	    ...
	    if (NCols <= TILE) {
		  // Compute
#pragma omp task firstprivate(M, NRows, NCols, y, x, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter)
		  for (int py = y; py < y + NRows; py++)
			for (int px = x; px < x + NCols; px++) {
