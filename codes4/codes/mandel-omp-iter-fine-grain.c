...
#pragma omp task firstprivate(x, y)
		  {
			...
			int equal_horizontal = 1;
			int equal_vertical = 1;
			M[y][x] = ref;
#pragma omp task shared(equal_vertical)
			for (int py = y + 1; py < y + TILE - 1; py++) {
			      ...
			      equal_vertical = equal_vertical & (ref == M[py][x + TILE - 1]);
			}
#pragma omp task shared(equal_horizontal)
			for (int px = x; px < x + TILE; px++) {
			      ...
			      equal_horizontal = equal_horizontal & (ref == M[y + TILE - 1][px]);
			}
#pragma omp taskwait
			if (equal_vertical & equal_horizontal) {
			      long color = (long)((ref - 1) * scale_color) + min_color;
			      if (output2histogram) {
#pragma omp atomic
			        histogram[ref - 1]+=(TILE*TILE);
			      }
			      for (int py = y; py < y + TILE; py++)
#pragma omp task firstprivate(py, x, ref, color)
				    for (int px = x; px < x + TILE; px++) {
					  M[py][px] = ref;
					  if (output2display) {
...
#pragma omp critical(display_output)
			      ...
			      // Compute
			      for (int py = y; py < y + TILE; py++)
#pragma omp task firstprivate(py, x)
				    for (int px = x; px < x + TILE; px++) {
