      ...
      for (int y = 0; y < ROWS; y += TILE) {
	    for (int x = 0; x < COLS; x += TILE) {
		  ...
		  for (int px = x; px < x + TILE; px++) {
			...
			equal_horizontal = equal_horizontal & (M[y][x] == M[y + TILE - 1][px]);
			equal_horizontal = equal_horizontal & (M[y][x] == M[y][px]);
		  }
		  ...
		  for (int py = y; py < y + TILE; py++) {
			...
			equal_vertical = equal_vertical & (M[y][x] == M[py][x + TILE - 1]);
			equal_vertical = equal_vertical & (M[y][x] == M[py][x]);
		  }
		  tareador_end_task("check_vertical");

		  //check if we can avoid computation of tile
		  tareador_start_task("tile_filling");
		  equal = equal_horizontal & equal_vertical;
		  if (equal) {
			if (output2histogram)
			...
			int val = M[y][x];
			for (int py = y; py < y + TILE; py++) {
				  tareador_start_task("tile_copy_borders");
			      for (int px = x; px < x + TILE; px++) {
				    M[py][px] = val;
				    if (output2display) {
				  ...
				  tareador_end_task("tile_copy_borders");
            }
		  } else {
			// computation of tile
			for (int py = y; py < y + TILE; py++) {
				  tareador_start_task("tile_computation");
			      for (int px = x; px < x + TILE; px++) {
				  ...
				  tareador_end_task("tile_computation");
            }
		  }
		  tareador_end_task("tile_filling");
