...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter, int depth)
{
      ...
      if (equal) {
	    ...
	    // Fill in matrix positions with the same value
            {
		  long color = (long)((value - 1) * scale_color) + min_color;
		  if (output2histogram) {
#pragma omp atomic
			histogram[value - 1] += (NRows * NCols);
		  }
		  for (int py = y; py < y + NRows; py++)
			for (int px = x; px < x + NCols; px++) {
			      M[py][px] = value;
...
#pragma omp critical(display_output)
            ...
            if (NRows > TILE) {
                  ...
                  if (depth <= CUTOFF) {
#pragma omp task firstprivate(M, hRows, hCols, y0, x0, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth)
                        mandel_tiled_rec(M, hRows, hCols, y0, x0,
                                          CminR, CminI, CmaxR, CmaxI,
                                          scale_real, scale_imag, maxiter, depth + 1);
#pragma omp task firstprivate(M, hRows, hCols, y1, x0, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth)
                        mandel_tiled_rec(M, hRows, hCols, y1, x0,
                                          CminR, CminI, CmaxR, CmaxI,
                                          scale_real, scale_imag, maxiter, depth + 1);
#pragma omp task firstprivate(M, hRows, hCols, y0, x1, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth)
                        mandel_tiled_rec(M, hRows, hCols, y0, x1,
                                          CminR, CminI, CmaxR, CmaxI,
                                          scale_real, scale_imag, maxiter, depth + 1);
#pragma omp task firstprivate(M, hRows, hCols, y1, x1, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth)
                  ...
                  if (depth <= CUTOFF) {
#pragma omp task firstprivate(M, NRows, hCols, y0, x0, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth)
                        mandel_tiled_rec(M, NRows, hCols, y0, x0,
                                          CminR, CminI, CmaxR, CmaxI,
                                          scale_real, scale_imag, maxiter, depth + 1);
#pragma omp task firstprivate(M, NRows, hCols, y0, x1, CminR, CminI, CmaxR, CmaxI, scale_real, scale_imag, maxiter, depth)
